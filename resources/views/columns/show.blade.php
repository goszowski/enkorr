		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="sidebar-block xs-mt-20">
					...
					@foreach($last_columns as $k=>$last_column)
						<a href="{{ lPath($last_column->node->absolute_path) }}" class="publication-sidebar_item xs-mt-10">
							@if(! $k)
								<div class="sidebar_item__img">
									<img src="{{ asset('gallery/' . $last_column->image) }}" alt="{{ $last_column->name }}">
									@if($last_column->is_exclusive)
										<span class="text-uppercase exclusive-public">{{ __('Эксклюзив') }}</span>
									@endif
								</div>
							@endif
							<div class="publication-sidebar_item__descr">
								<h3 class="publication-sidebar_item__title">{{ $last_column->name }}</h3>
								<p class="publication-sidebar_item__text xs-mt-15">{{ $last_column->announce }}</p>
								<p class="comment-text_time text-uppercase"><time>{{ $last_column->pubdate->format('d.m.Y, H:i') }}</time></p>
							</div>
						</a>
					@endforeach
				</div>
