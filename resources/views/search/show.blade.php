...
@include('_partials.seo2')
	...
	@else
		...
		@else
			<div class="news-list_oneday">
				@foreach($results as $results_item)
					<a href="{{ lPath($results_item->node->absolute_path) }}" class="main-news_sidebar news-list-item xs-pb-10 xs-pt-25">
						<div class="row">
							<div class="col-sm-2">
								<p class="comment-text_time text-uppercase small-text"><time><small>{{ $results_item->pubdate->format('d.m.Y, H:i') }}</small></time></p>
							</div>
							<div class="col-sm-10">
			...
			{!! $results->appends(['term'=>request('term'), 'titles'=>request('titles'), 'announces'=>request('announces'), 'texts'=>request('texts')])->render() !!}
		@endif
	@endif
@endsection
