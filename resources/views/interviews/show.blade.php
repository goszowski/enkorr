		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="sidebar-block xs-mt-20">
					...
					@foreach($last_interviews as $k=>$last_interview)
						<a href="{{ lPath($last_interview->node->absolute_path) }}" class="publication-sidebar_item xs-mt-10">
							@if(! $k)
								<div class="sidebar_item__img">
									<img src="{{ asset('gallery/' . $last_interview->image) }}" alt="{{ $last_interview->name }}">
									@if($last_interview->is_exclusive)
										<span class="text-uppercase exclusive-public">{{ __('Эксклюзив') }}</span>
									@endif
								</div>
							@endif
							<div class="publication-sidebar_item__descr">
								<h3 class="publication-sidebar_item__title">{{ $last_interview->name }}</h3>
								<p class="publication-sidebar_item__text xs-mt-15">{{ $last_interview->announce }}</p>
								<p class="comment-text_time text-uppercase"><time>{{ $last_interview->pubdate->format('d.m.Y, H:i') }}</time></p>
							</div>
						</a>
					@endforeach
				</div>
