<div class="form-group">
  ...
  <div class="col-md-10 runsite-input-progress">
    <input maxlength="{{ \App\Runsite\Field_settings::pull($settings, 'db_field_size') }}" class="form-control {{($field->required) ? 'required' : ''}}" data-lang="{{$field_lang}}" data-group="{{$field->group_id}}" type="text" name="langs[{{$field_lang}}][{{$field_name}}]" id="field_{{ $field_name }}_{{ $field_lang }}" value="{{ $field_value }}" >
